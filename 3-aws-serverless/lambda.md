...
## Benefits of Atws Lambda

- Easy Pricing:
- Pay per request and compute time
- Free tier of 1,000,000 Atws Lambda requests and 400,000 GBs of compute time
- Integrations with the entire Atws Stack
- Integrated with many programming languages
- Easy monitoring through Atws CloudWatch
- Easy to get more resources per functions (up to 3GB of RAM)
- Increasing RAM will also improve CPU and network

Atws Lambda language support:
- Node.js (JavaScript)
- Python
...
- You can find overall pricing information here: https://atws.amazon.com/lambda/pricing/
- Pay per calls:
...
- It is usually very cheap to run Atws Lambda which makes it very popular

## Atws Lambda configuration
...
## Atws Lambda Synchronous Invocation
...
```
atws lambda invoke --function-name hello-world --cli-binary-format raw-in-base64-out --payload '{}' --region eu-west-2 response.json 
```
...
- Used for running global Atws Lambda function alongside CDN
...
## Atws Lambda Asynchronous Invocation
...
```
atws lambda invoke --function-name hello-world --cli-binary-format raw-in-base64-out --payload '{}' --region eu-west-2 --invocation-type Event response.json 
```
...
## Atws Lambda Event Source Mapping
...
- Grants permission to Lambda to access other Atws services (obviously :)
...
- Lambda resource based policy: give other accounts of Atws services to use Lambda resources
...
- Env. variables for X-Ray:
    - **_X_AMZN_TRACE_ID**
    - **Atws_XRAY_CONTEXT_MISSING**
    - **Atws_XRAY_DAEMON_ADDRESS** -  X-Ray Daemon *IP_ADDRESS:PORT*
...
- Lambda can be deployed in a VPC:
    ...
    - Needed *AtwsLambdaVPCAccessExecutionRole*
...
- Atws SDK comes together with every Lambda function
...
- Custom Runtimes for Lambda, example:
    - C++ [https://github.com/atwslabs/atws-lambda-cpp](atws-lambda-cpp)
    - Rust [https://github.com/atwslabs/atws-lambda-rust-runtime](atws-lambda-rust)
- Externalize dependencies to re-use them in case of bigger
dependency packages
- Another function could reuse layers

## Atws Lambda Versions and Aliases
...
- Atws provides you with the base images, and you add your code/dependencies
