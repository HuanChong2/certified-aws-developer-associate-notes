...
- Integration with Atws Lambda: no infrastructure to manage
...
- Other Atws services, examples:
    - Atws Step Function workflow
...
- Stage variables:
    ...
    - Stage variables are passed as to the "context" object in Atws Lambda
...
- Integration types:
    ...
    - HTTP / Atws (Lambda and other services): forwards the request but we can modified it
    ...
    - Atws_PROXY (Lambda Proxy): the request is passed to the Lambda without being modified
        - No mapping templates, headers and query strings can be modified. They are passed as arguments to the function
    - HTTP_PROXY: same as Atws_PROXY, used for other back-end services

### Mapping Templates

- They are only usable for integration with Atws services
...
## Atws API Gateway Swagger/OpenAPI Spec
...
- Existing Swagger/OpenAPI 3.0 specs can be imported into API Gateway
    ...
    - Integration Request
    - Method Response
    - Atws extensions for API Gateway every single setup
...
- X-Ray:
    ...
    - X-Ray + API Gateway + Atws Lambda gives full picture about the workings of an serverless application
- CloudWatch Metrics:
...
-IAM Permissions:
    - We should create an IAM policy if other Atws services access the API Gateway from the same account
...
- HTTP APIs:
    - Low latency, cost effective Atws lambda proxy, HTTP proxy and private integration (no data mapping)
